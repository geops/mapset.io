...
function AnnualPromo() {
  ...
  return (
    <div className="pointer-events-none">
      <svg
        className="absolute right-10 top-[-20px] sm:right-[-20px] sm:top-[-10px]"
        width="49"
        height="28"
        viewBox="0 0 49 28"
        fill="none"
  ...
  return (
    <div className="xl:w-5/6 xl:mx-auto">
      <div className="relative w-[clamp(300px,40vw,500px)] mb-8 mx-auto bg-blue-700 rounded-full flex gap-0 items-center px-6 tracking-[.14px] leading-6 font-hero">
        ...
        <Button
          className={`${billingSwitcherBtnClassName} mr-2${
            isAnnualBilling ? " bg-blue-800" : ""
          }`}
      ...
      <div className="w-full border-[3px] rounded-2xl bg-white p-6 font-hero my-6">
        ...
        <ul>
          <ListItem
            title={t("pricing.add_on_coop")}
            content={t("pricing.add_on_coop-description")}
          />
          <ListItem
            title={t("pricing.add_on_defaultplans")}
            content={t("pricing.add_on_defaultplans-description")}
          />
        </ul>
        <br />
        <div className="w-full flex flex-wrap sm:flex-nowrap gap-6">
          {products.map((product) => {
            return (
              <div
                key={product.tier}
                className="sm:basis-1/4 w-full sm:w-auto text-center rounded-2xl p-6 text-blue-700 border border-[#8FCCFE] bg-[#F1F9FE]"
              >
                <p className={`mb-3 text-sm font-medium	${inter.className}`}>
                  {product.tier}
                </p>
